...
│   ├── base_average_results.csv     # 静态 ECMP 基线平均结果
...
为了验证 AI 预测式、阈值响应式和静态 ECMP（base 基线）的差异，设计了“概率哈希碰撞与渐进突发流”的极端测试场景：
...
| 性能指标 | base Static Hash (基线) | Threshold Reactive (阈值) | Predictive Proactive (AI预测) |
| :--- | :---: | :---: | :---: |
...
#### 结果分析：
1. **拥塞重分配能力**：AI 预测式算法取得了最佳的总体网络性能。其突发流丢包率从 base 基线的 28.77% 降至 **12.47%**，较阈值响应式（16.76%）也减少了约 4.3 个百分点。
...
3. **吞吐量保障**：突发流吞吐量从 base 基线 1.06 Mbps 上升至 **1.38 Mbps**，有效保障了高优先级突发流量的传输带宽。
