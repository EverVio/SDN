...
│
├── figures/                           # 可视化输出（17 张 PNG，180 DPI）
│   ├── 1_spatiotemporal_heatmap.png   # 全网链路时空利用率热力图
│   ├── 2_key_link_utilization.png     # 关键骨干链路利用率曲线
│   ├── 3_total_network_load.png       # 全网总负载时间线
│   ├── 3_training_convergence.png     # MLP 训练收敛曲线
│   ├── 4_traffic_correlation_matrix.png # 链路流量相关矩阵
│   ├── 4_true_vs_predicted_scatter.png # 真实 vs 预测散点图
│   ├── 5_single_link_tracking.png     # 单链路预测追踪
│   ├── 6_residual_distribution.png    # 预测残差分布
│   ├── 7_spatial_error_distribution.png # 空间误差分布
│   ├── 8_hierarchical_error_distribution.png # 层级误差分布
...
| :--- | :---: | :---: | :---: | :---: |
| **突发流丢包率 (%)** | 28.77 | 16.76 | **11.54** | -60.0% |
| **突发流平均抖动 (ms)** | 23.98 | 33.16 | **27.45** | +14.4% |
| **突发流平均吞吐量 (Mbps)** | 1.06 | 1.27 | **1.27** | +19.8% |
| **全网最低丢包率 (%)** | 17.78 | 9.33 | **0.00** | -100% |
| **全网最高丢包率 (%)** | 46.35 | 72.00 | **21.00** | -54.7% |
...
|:---:|:---|:---:|:---:|:---:|:---:|
| Flow 1 | h0_0 → h3_0 | 39.63 | 21.49 | **21.00** | -47.0% |
| Flow 2 | h0_1 → h3_1 | 30.19 | 27.48 | **14.00** | -53.6% |
| Flow 3 | h0_2 → h3_2 | 21.34 | 9.33 | **0.00** | -100% |
| Flow 4 | h0_0 → h3_0 | 17.78 | 11.74 | **8.20** | -53.9% |
| Flow 5 | h0_1 → h3_1 | 33.84 | 16.05 | **8.10** | -76.1% |
| Flow 6 | h0_2 → h3_2 | 34.10 | 17.40 | **17.00** | -50.1% |
| Flow 7 | h0_0 → h3_0 | 29.50 | 15.61 | **3.10** | -89.5% |
| Flow 8 | h0_1 → h3_1 | 46.35 | 22.77 | **4.00** | -91.4% |
| Flow 9 | h0_2 → h3_2 | 26.77 | 12.99 | **2.90** | -89.2% |

#### 背景流平均抖动 (ms)

| 流编号 | base | threshold | predictive |
|:---:|:---:|:---:|:---:|
| Flow 1 | 17.26 | 31.76 | **12.27** |
| Flow 2 | 17.21 | 29.72 | **14.69** |
| Flow 3 | 7.97 | 32.84 | 19.98 |
| Flow 4 | 12.22 | 31.53 | 19.01 |
| Flow 5 | 9.72 | 33.52 | 19.59 |
| Flow 6 | 8.71 | 29.53 | **13.84** |
| Flow 7 | 9.25 | 34.09 | 19.62 |
| Flow 8 | 21.15 | 32.38 | **22.06** |
| Flow 9 | 8.11 | 28.28 | 18.51 |

#### 背景流平均吞吐量 (Mbps，期望值 0.5 Mbps)

| 流编号 | base | threshold | predictive |
|:---:|:---:|:---:|:---:|
| Flow 1 | 0.29 | 0.39 | **0.38** |
| Flow 2 | 0.34 | 0.36 | **0.41** |
| Flow 3 | 0.38 | **0.45** | **0.48** |
| Flow 4 | 0.40 | **0.44** | **0.44** |
| Flow 5 | 0.32 | 0.42 | **0.44** |
| Flow 6 | 0.32 | 0.41 | **0.40** |
| Flow 7 | 0.34 | 0.42 | **0.47** |
| Flow 8 | 0.26 | 0.39 | **0.46** |
| Flow 9 | 0.36 | 0.44 | **0.47** |

#### 突发流聚合指标

| 指标 | base | threshold | predictive | AI vs base |
|:---|:---:|:---:|:---:|:---:|
| **丢包率 (%)** | 28.77 | 16.76 | **11.54** | -60.0% |
| **抖动 (ms)** | 23.98 | 33.16 | 27.45 | +14.4% |
| **吞吐量 (Mbps)** | 1.06 | 1.27 | **1.27** | +19.8% |
...
#### AI 预测式（predictive）突发流数据

当前实验仅运行 1 轮（数据文件显示 iteration=1），突发流指标为：
- **丢包率**: 11.54%
- **抖动**: 27.45 ms
- **吞吐量**: 1.27 Mbps

> 注：predictive 组仅采集了 1 轮数据，统计显著性有限。但从单轮结果看，丢包率（11.54%）已低于 base 30 轮的最差值（47.14%）和大多数轮次。
...
从逐流数据看，AI 预测式在以下流上实现了接近零丢包：
- Flow 3: **0.00%**（base: 21.34%，threshold: 9.33%）
- Flow 7: **3.10%**（base: 29.50%，threshold: 15.61%）
- Flow 8: **4.00%**（base: 46.35%，threshold: 22.77%）
- Flow 9: **2.90%**（base: 26.77%，threshold: 12.99%）

这表明 MLP 模型成功预测了拥塞趋势，并在拥塞实际发生前就开始迁移流量，避免了这些流的严重拥塞。
...
| **尾部风险** | 极端轮次丢包 > 80% | 极端轮次丢包 > 40% | 单轮丢包稳定在 12% 左右 |
...
#### 图表 3：全网总负载（`3_total_network_load.png`）

- 所有端口利用率的算术平均
- 标注实验事件时间点

#### 图表 4：流量相关矩阵（`4_traffic_correlation_matrix.png`）
...
#### 图表 3：训练收敛曲线（`3_training_convergence.png`）

- 双轴：训练损失（红色，左轴）+ 验证 R2（蓝色，右轴）
- 标记最佳验证 R2 epoch
- 高亮早停后的额外训练区域

#### 图表 4：真实 vs 预测散点（`4_true_vs_predicted_scatter.png`）
...
#### 图表 5：单链路追踪（`5_single_link_tracking.png`）
...
#### 图表 6：残差分布（`6_residual_distribution.png`）

- 预测残差（true - predicted）直方图 + KDE 曲线
- 标注：均值、标准差、偏度、极端残差比例（|r| > 0.3%）
- 高亮偏斜方向

#### 图表 7：空间误差分布（`7_spatial_error_distribution.png`）

- 每条链路的 RMSE 柱状图（降序）
- Top-20 单独显示，其余归为 "Others (Mean)"
- 按拓扑层着色：红=Core，橙=Agg，绿=Edge

#### 图表 8：层级误差小提琴图（`8_hierarchical_error_distribution.png`）
