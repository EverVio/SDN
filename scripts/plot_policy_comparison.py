            ...
            for bar, v in zip(bars, vals):
                if v > 0:
                    if col == "avg_jitter_ms":
                        y_offset = (0.01 + i * 0.06) * (ax.get_ylim()[1] - ax.get_ylim()[0])
                    else:
...
def plot_weight_evolution():
    ...
    for i, (ax, dpid) in enumerate(zip(axes, dpids)):
        ...
        w_smooth = 5
        smoothed_w3 = sub_df["port3_weight"].rolling(w_smooth, min_periods=1, center=True).mean()
        smoothed_w4 = sub_df["port4_weight"].rolling(w_smooth, min_periods=1, center=True).mean()
...
def plot_dual_axis_coevolution():
    ...
    if not (os.path.exists(weights_path) and os.path.exists(traffic_path)):
        print("Warning: Missing weights or traffic files. Skipping dual-axis co-evolution.")
        return
    ...
    df_t_sub3 = df_t[(df_t["dpid"] == dpid_target) & (df_t["port_no"] == 3)].sort_values("timestamp")
    df_t_sub4 = df_t[(df_t["dpid"] == dpid_target) & (df_t["port_no"] == 4)].sort_values("timestamp")
    ...
    ax1.plot(t_w, smooth(df_w_sub["port3_weight"]), color=color_w, linewidth=1.8, label="Port 3 Weight")
    ...
    ax1_twin = ax1.twinx()
    ax1_twin.plot(t_t3, smooth(df_t_sub3["utilization"]), color=color_u, linewidth=1.5, linestyle="--", label="Port 3 Utilization")
    ...
    ax1.grid(True, alpha=0.3)
    ax1.set_title(f"Switch S{dpid_target} Port 3 Co-evolution", fontsize=12, fontweight="bold")
    ...
    ax2.plot(t_w, smooth(df_w_sub["port4_weight"]), color=color_w, linewidth=1.8, label="Port 4 Weight")
    ...
    ax2_twin = ax2.twinx()
    ax2_twin.plot(t_t4, smooth(df_t_sub4["utilization"]), color=color_u, linewidth=1.5, linestyle="--", label="Port 4 Utilization")
    ...
    ax2.grid(True, alpha=0.3)
    ax2.set_title(f"Switch S{dpid_target} Port 4 Co-evolution", fontsize=12, fontweight="bold")
...
def plot_pareto_tradeoff():
    ...
    for g in groups:
        ...
        if not burst_row.empty:
            ax.annotate(
                ...
                xy=(burst_row["avg_loss_pct"].values[0], burst_row["avg_bandwidth_mbps"].values[0]),
                xytext=(burst_row["avg_loss_pct"].values[0] + 1.2, burst_row["avg_bandwidth_mbps"].values[0] - 0.03),
                fontsize=8.5,
                fontweight="bold",
    ...
    ideal_bw = max_bw  # The theoretical peak capacity achieved by any path in this dataset

    # 2. Draw the Ideal Point and dynamic annotation
    ax.plot(0.0, ideal_bw, marker="*", color="gold", markersize=16, label="Ideal Point", markeredgecolor="black", zorder=5)
    ax.text(1.2, ideal_bw - 0.02, f"Ideal Point\n(0% loss, max {ideal_bw:.2f} Mbps)", color="#b7950b", fontsize=9.5, fontweight="bold")
...
def plot_flow_fairness_radar():
    ...
    for g in groups:
        ...
        ax.plot(angles, values, color=POLICY_COLORS[g], linewidth=2, label=POLICY_LABELS[g])
