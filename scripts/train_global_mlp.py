...
def train_model():
    ...
    # Temporal split: no shuffle to prevent future data leakage
    ...
    # StandardScaler is critical for MLP convergence (gradient-based optimizer)
    ...
    #  - hidden (128, 64): sufficient for 120 -> 48 nonlinear mapping
    #  - early_stopping: auto-split validation to prevent overfitting
    #  - max_iter=1000: generous budget for convergence
    model = MLPRegressor(
        hidden_layer_sizes=(128, 64),
        activation='relu',
        solver='adam',
        alpha=0.001,           # L2 regularization (default 0.0001)
        max_iter=1000,
    ...
    Y_pred = np.clip(Y_pred, 0.0, 1.0)  # physical constraint: utilization in [0, 1]
    ...
    # Save model bundle: model + scalers + metadata
    # link_keys ensures online inference uses identical feature ordering
